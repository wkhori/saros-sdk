  ...
  async swap(params) {
    try {
      ...
      const swapIx = await this.ammProgram.methods
        .swap(new BN$1(amount.toString()), new BN$1(minAmountOut.toString()))
        .accountsPartial({
...
async function buildCreatePairTransaction(args) {
  ...
  if (shouldCreateFeeAccountAta) {
    tx.add(
      spl2.createAssociatedTokenAccountIdempotentInstruction(payer, feeAccount, feeOwner, lpMintKeypair.publicKey)
    );
  }
  tx.add(
...
var SarosFarm = class {
  ...
  async stake(params) {
    ...
    const userStakingTokenAccount =
      ...
      (await spl2.getAssociatedTokenAddress(this.poolAccount.stakingTokenMint, payer));
    ...
    const stakeIx = await this.farmProgram.methods
      .stakePool(new BN$1(amount.toString()))
      .accounts({
        pool: this.poolAddress,
  ...
  async unstake(params) {
    ...
    const unstakeIx = await this.farmProgram.methods
      .unstakePool(new BN$1(amount.toString()))
      .accounts({
        pool: this.poolAddress,
        poolAuthority,
        poolStakingTokenAccount: this.poolAccount.stakingTokenAccount,
  ...
  async claimReward(params) {
    ...
    const userRewardTokenAccount =
      params.userRewardTokenAccount || (await spl2.getAssociatedTokenAddress(poolRewardAccount.rewardTokenMint, payer));
