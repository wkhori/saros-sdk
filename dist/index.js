...
import { AnchorProvider, Program, BN as BN$1 } from '@coral-xyz/anchor';
import BN from 'bn.js';
import * as spl2 from '@solana/spl-token';
...
var SAROS_FEE_OWNER = new PublicKey("FDbLZ5DRo61queVRH9LL1mQnsiAoubQEnoCRuPEmH9M8");
var AMM_PROGRAM_IDS = {
  ["mainnet" /* MAINNET */]: new PublicKey("SSwapUtytfBdBn1b9NUGG6foMVPtcWgpRU32HToDUZr"),
  ["devnet" /* DEVNET */]: new PublicKey("SSStkgZHW17LRbGUFSDQqzZ4jMXpfmVxHDbWwMaFEXE")
};
var FARM_PROGRAM_IDS = {
  ["mainnet" /* MAINNET */]: new PublicKey("FARMr8rFJohG2CXFWKKmj8Z3XAhZNdYZ5CE3TKeWpump"),
  ["devnet" /* DEVNET */]: new PublicKey("SFFxHvYKTBgC7XYEZDua6m28NZo9fNnDRCEtM7AHR4m")
};
var SWAP_ACCOUNT_SIZE = 324;
var DEFAULT_FEES = {
  tradeFeeNumerator: new BN(0),
  tradeFeeDenominator: new BN(1e4),
  ownerTradeFeeNumerator: new BN(30),
  ownerTradeFeeDenominator: new BN(1e4),
...
var SarosBaseService = class {
  constructor(config) {
    ...
    const provider = new AnchorProvider(this.connection, {}, AnchorProvider.defaultOptions());
    const programId = AMM_PROGRAM_IDS[config.mode];
    this.ammProgram = new Program({ ...amm_default, address: programId.toBase58() }, provider);
  }
  getDexName() {
    return "Saros AMM";
  }
...
function decodePairAccount(data) {
  try {
    ...
    return {
      ...
      fees: {
        tradeFeeNumerator: new BN$1(decoded.tradeFeeNumerator.toString()),
        tradeFeeDenominator: new BN$1(decoded.tradeFeeDenominator.toString()),
        ownerTradeFeeNumerator: new BN$1(decoded.ownerTradeFeeNumerator.toString()),
        ownerTradeFeeDenominator: new BN$1(decoded.ownerTradeFeeDenominator.toString()),
        ownerWithdrawFeeNumerator: new BN$1(decoded.ownerWithdrawFeeNumerator.toString()),
        ownerWithdrawFeeDenominator: new BN$1(decoded.ownerWithdrawFeeDenominator.toString()),
        hostFeeNumerator: new BN$1(decoded.hostFeeNumerator.toString()),
        hostFeeDenominator: new BN$1(decoded.hostFeeDenominator.toString())
      },
...
var SarosAMMPair = class extends SarosBaseService {
  ...
  async swap(params) {
    try {
      ...
      const userTokenA = params.userTokenX || await spl2.getAssociatedTokenAddress(tokenAMint, payer, true);
      const userTokenB = params.userTokenY || await spl2.getAssociatedTokenAddress(tokenBMint, payer, true);
      ...
      const swapIx = await this.ammProgram.methods.swap(new BN$1(amount.toString()), new BN$1(minAmountOut.toString())).accountsPartial({
        ...
        tokenProgramInfo: spl2.TOKEN_PROGRAM_ID
      }).instruction();
      tx.add(swapIx);
      return tx;
    } catch (error) {
  ...
  async addLiquidity(params) {
    try {
      ...
      const userTokenA = params.userTokenX || await spl2.getAssociatedTokenAddress(tokenAMint, payer, true);
      const userTokenB = params.userTokenY || await spl2.getAssociatedTokenAddress(tokenBMint, payer, true);
      const userLpToken = params.userLpToken || await spl2.getAssociatedTokenAddress(poolMint, payer, true);
      const depositIx = await this.ammProgram.methods.depositAllTokenTypes(
        new BN$1(poolTokenAmount.toString()),
        new BN$1(maximumTokenA.toString()),
        new BN$1(maximumTokenB.toString())
      ).accountsPartial({
        swapInfo: this.pairAddress,
        authorityInfo: this.poolAuthority,
        userTransferAuthorityInfo: payer,
  ...
  async removeLiquidity(params) {
    try {
      ...
      const withdrawIx = await this.ammProgram.methods.withdrawAllTokenTypes(
        new BN$1(poolTokenAmount.toString()),
        new BN$1(minimumTokenA.toString()),
        new BN$1(minimumTokenB.toString())
      ).accountsPartial({
        swapInfo: this.pairAddress,
        authorityInfo: this.poolAuthority,
        userTransferAuthorityInfo: payer,
  ...
  async buildPairMetadata() {
    ...
    const tokenAData = tokenAInfo ? spl2.AccountLayout.decode(tokenAInfo.data) : null;
    const tokenBData = tokenBInfo ? spl2.AccountLayout.decode(tokenBInfo.data) : null;
    const [mintAInfo, mintBInfo, poolMintInfo] = await Promise.all([
      spl2.getMint(this.connection, tokenAMint),
      spl2.getMint(this.connection, tokenBMint),
      spl2.getMint(this.connection, poolMint)
    ]);
...
async function buildCreatePairTransaction(args) {
  ...
  const poolTokenA = await spl2.getAssociatedTokenAddress(tokenAMint, poolAuthority, true);
  const poolTokenB = await spl2.getAssociatedTokenAddress(tokenBMint, poolAuthority, true);
  const userLpTokenAccount = await spl2.getAssociatedTokenAddress(lpMintKeypair.publicKey, payer);
  const feeAccount = await spl2.getAssociatedTokenAddress(lpMintKeypair.publicKey, feeOwner);
  const userTokenA = await spl2.getAssociatedTokenAddress(tokenAMint, payer);
  const userTokenB = await spl2.getAssociatedTokenAddress(tokenBMint, payer);
  ...
  const [swapAccountRent, mintRent, { blockhash }] = await Promise.all([
    ...
    connection.getMinimumBalanceForRentExemption(spl2.MINT_SIZE),
  ...
  tx.add(
    SystemProgram.createAccount({
      ...
      space: spl2.MINT_SIZE,
      lamports: mintRent,
      programId: spl2.TOKEN_PROGRAM_ID
    }),
    spl2.createInitializeMintInstruction(lpMintKeypair.publicKey, 2, poolAuthority, null)
  );
  tx.add(
    spl2.createAssociatedTokenAccountInstruction(payer, poolTokenA, poolAuthority, tokenAMint),
    spl2.createAssociatedTokenAccountInstruction(payer, poolTokenB, poolAuthority, tokenBMint)
  );
  tx.add(
    spl2.createAssociatedTokenAccountIdempotentInstruction(payer, userLpTokenAccount, payer, lpMintKeypair.publicKey)
  );
  if (shouldCreateFeeAccountAta) {
    tx.add(spl2.createAssociatedTokenAccountIdempotentInstruction(payer, feeAccount, feeOwner, lpMintKeypair.publicKey));
  }
  tx.add(
    spl2.createTransferInstruction(userTokenA, poolTokenA, payer, initialTokenAAmount),
    spl2.createTransferInstruction(userTokenB, poolTokenB, payer, initialTokenBAmount)
  );
  tx.add(
    SystemProgram.createAccount({
      fromPubkey: payer,
...
var SarosFarm = class {
  ...
  constructor(config, poolAddress, poolType = "farm") {
    ...
    this.farmProgram = new Program({ ...FARM_IDL, address: programId.toBase58() }, provider);
  }
  /**
  ...
  async stake(params) {
    ...
    const userStakingTokenAccount = params.userStakingTokenAccount || await spl2.getAssociatedTokenAddress(this.poolAccount.stakingTokenMint, payer);
    ...
    const stakeIx = await this.farmProgram.methods.stakePool(new BN$1(amount.toString())).accounts({
      pool: this.poolAddress,
  ...
  async unstake(params) {
    ...
    const unstakeIx = await this.farmProgram.methods.unstakePool(new BN$1(amount.toString())).accounts({
      pool: this.poolAddress,
      poolAuthority,
      poolStakingTokenAccount: this.poolAccount.stakingTokenAccount,
      user: payer,
  ...
  async claimReward(params) {
    ...
    const userRewardTokenAccount = params.userRewardTokenAccount || await spl2.getAssociatedTokenAddress(poolRewardAccount.rewardTokenMint, payer);
    const userRewardAccountInfo = await this.connection.getAccountInfo(userRewardTokenAccount);
    if (!userRewardAccountInfo) {
      const createATAIx = spl2.createAssociatedTokenAccountInstruction(
        payer,
...
export { AMM_PROGRAM_IDS, CURVE_TYPE_MAP, DEFAULT_FEES, DEFAULT_SWAP_CALCULATOR, FARM_IDL, FARM_PROGRAM_IDS, MODE, SAROS_FEE_OWNER, SWAP_ACCOUNT_SIZE, SarosAMM, SarosAMMError, SarosAMMErrorCode, SarosAMMPair, SarosAPIService, SarosBaseService, SarosFarm, SwapCurveType, calculatePriceImpact, calculateSwapOutput, deriveFarmPoolAuthority, deriveFarmPoolRewardAuthority, deriveFarmUserPoolAddress, deriveFarmUserPoolRewardAddress, derivePoolAuthority, getMinOutputWithSlippage };
