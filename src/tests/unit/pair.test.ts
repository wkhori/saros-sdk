...
describe('SarosAMMPair Unit Tests', () => {
  ...
  describe('Liquidity Operations', () => {
    ...
    it('should build addLiquidity transaction without submitting', async () => {
      const mockPayer = PublicKey.unique();
      ...
      expect(depositIx.programId.toBase58()).toBe('SSwapUtytfBdBn1b9NUGG6foMVPtcWgpRU32HToDUZr');

      console.log('Add liquidity transaction built:');
      console.log('  Instructions:', tx.instructions.length);
      console.log('  Pool token amount: 1_000_000_000');
    });

    it('should build removeLiquidity transaction without submitting', async () => {
      const mockPayer = PublicKey.unique();
      ...
      const withdrawIx = tx.instructions[0];
      expect(withdrawIx.programId.toBase58()).toBe('SSwapUtytfBdBn1b9NUGG6foMVPtcWgpRU32HToDUZr');

      console.log('Remove liquidity transaction built:');
      console.log('  Instructions:', tx.instructions.length);
      console.log('  Pool token amount: 1_000_000_000');
    });

    it('should validate zero amount for addLiquidity', async () => {
      const mockPayer = PublicKey.unique();
    ...
    it('should validate zero amount for removeLiquidity', async () => {
      const mockPayer = PublicKey.unique();
    ...
    it('should validate maximum amounts for addLiquidity', async () => {
      const mockPayer = PublicKey.unique();
    ...
    it('should support optional transaction parameter for composability', async () => {
      const mockPayer = PublicKey.unique();
      const existingTx = await pair.swap({
        payer: mockPayer,
        amount: 1_000_000n,
      ...
      const composedTx = await pair.addLiquidity({
        payer: mockPayer,
        poolTokenAmount: 1_000_000_000n,
        maximumTokenA: 10_000_000n,
        maximumTokenB: 10_000_000n,
      ...
      expect(composedTx.instructions.length).toBe(2); // swap + addLiquidity
      console.log('Transaction composition works:', composedTx.instructions.length, 'instructions');
    });
  });
